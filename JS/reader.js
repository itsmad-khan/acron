...
const DEFAULT_CHAPTERS = ['Chapter 1', 'Chapter 2', 'Chapter 3'];
...
/* ─────────────────────────────────────────
   Book PDF links per board / class / subject
   NOTE: empty string '' means "not yet available"
───────────────────────────────────────── */
const BOOK_LINKS = {
  punjab: {
    '11': {
...
function getLocalUser() {
  try {
    ...
    console.error('[Reader] Could not parse local user data.');
    return null;
  }
}
...
function initReader() {
  ...
  setText('book-title', subject);
  setText('book-meta',
    `Class ${user.cls} — ${BOARD_NAMES[user.board] ?? user.board}`);
...
/* ─────────────────────────────────────────
   Chapter list
───────────────────────────────────────── */
function loadChapterList(subject) {
  const list = document.getElementById('chapters-list');
  if (!list) return;

  const chapters = CHAPTERS_BY_SUBJECT[subject] ?? DEFAULT_CHAPTERS;
  list.innerHTML = '';
  ...
  chapters.forEach((ch, i) => {
    ...
    btn.addEventListener('click', () => openChapter(btn, subject, ch, i + 1));
...
function openChapter(btn, subject, chapterName, chNum) {
  ...
  document.querySelectorAll('.chapter-item').forEach(b => {
  ...
  setText('current-chapter-name', chapterName);

  // Hide "pick a chapter" placeholder
  const pickState = chView?.querySelector('.pick-chapter-state');
  if (pickState) pickState.style.display = 'none';

  // Wire up "quiz this chapter" link
  ...
  // On mobile, auto-close the sidebar after selecting a chapter
  const sidebar = document.getElementById('reader-left');
...
function loadBookPDF(subject, chNum) {
  ...
  const { board, cls } = user;
  const url = BOOK_LINKS[board]?.[cls]?.[subject];

  const pdfWrap = document.getElementById('pdf-frame-wrap');

  if (url) {
    frame.src = `${url}#page=${chNum * 10}`;
    // Restore the iframe in case a "coming soon" message replaced it earlier
    if (pdfWrap && !pdfWrap.querySelector('#pdf-frame')) {
      pdfWrap.innerHTML = '';
    ...
    if (pdfWrap) {
      pdfWrap.innerHTML = `
        ...
        ">
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true" style="opacity:0.4">
            <path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/>
            <path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/>
          </svg>
          <p style="font-size:15px;color:var(--accent-light);font-weight:700;margin:0">
            Book link coming soon
          </p>
          <p style="font-size:13px;margin:0;max-width:280px;line-height:1.5">
            Visit your board's official website to find this book in the meantime.
          </p>
          <p style="font-size:13px;margin-top:4px;color:var(--accent-light)">
...
function saveChapterRead(subject, chNum) {
  ...
  if (!Array.isArray(user.chaptersRead)) user.chaptersRead = [];
  ...
  if (!user.chaptersRead.includes(key)) {
    ...
    try {
      ...
      console.warn('[Reader] Could not persist chaptersRead.');
    }

    // TODO: also sync to Firebase if firebase-config.js exposes a helper,
    // e.g. firebaseMarkChapterRead(uid, key) — currently local-only.
  }
}
...
function changeFont(dir) {
  ...
  // Note: cross-origin iframes (Google Drive/PCTB PDFs) cannot have their
  // internal font size changed by the parent page due to browser security.
  // This control is kept for future same-origin / custom PDF viewer support.
  const frame = document.getElementById('pdf-frame');
...
function toggleDark() {
  darkMode = !darkMode;
  // Defer to the shared theme system instead of hardcoding a color here
